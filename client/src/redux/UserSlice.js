...
export const AddToCart = createAsyncThunk('user/cart', async({id, stuff}) => {
	try {
		let result = await axios.post(`http://localhost:5000/user/${id}/cart`, stuff);
		return result;
	} catch (error) {
		console.log(error);
	}
...
export const AddToWishList = createAsyncThunk('user/wishlist', async({id, stuff_1}) => {
	try {
		let result = await axios.post(`http://localhost:5000/user/${id}/wishlist`, stuff_1);
		return result.data;
	} catch (error) {
		console.log(error);
	}
...
export const getUserOrders = createAsyncThunk('user/order', async(id) => {
    try {
        let result = await axios.get(`http://localhost:5000/user/${id}/order`);
		return result;
    } catch (error) {
        console.log(error);
    }
...
export const RemoveCartItem = createAsyncThunk('user/cartDel', async({userId, cartId}) =>{
	try {
		await axios.delete(`http://localhost:5000/user/cart/${userId}/${cartId}`);
	} catch (error) {
		console.log(error);
	}
})
...
export const RemoveWishListItem = createAsyncThunk('user/wishDel', async({userId, cartId}) =>{
	try {
		await axios.delete(`http://localhost:5000/user/wishlist/${userId}/${cartId}`);
	} catch (error) {
		console.log(error);
	}
})
