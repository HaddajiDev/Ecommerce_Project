...
export const AddOrder = createAsyncThunk('order/add', async ({ userId, products, totalAmount }) => {
    try {
        let result = await axios.post('http://ecommerce-back-beta.vercel.app/order/', { userId, products, totalAmount });
        return result.data;
    } catch (error) {
        console.log(error);
        throw error;
...
export const AllOrders = createAsyncThunk('order/all', async() => {
    try {
        let result = await axios.get('http://ecommerce-back-beta.vercel.app/order/');
        return result;
    } catch (error) {
        console.log(error);
    }
...
export const UpdateOrder_Put = createAsyncThunk('order/all', async({id, state}) => {
    try {
        let result = await axios.put(`http://ecommerce-back-beta.vercel.app/order/${id}`, state);
        return result;
    } catch (error) {
        console.log(error);
    }
